...
function getCookieLang(): Locale {
  ...
  const val = m?.[1];
  if (val === "fr" || val === "pt" || val === "en" || val === "ar") return val;
  return "en";
}
...
export function LanguageProvider({
  ...
  // On mount, prefer cookie over server-passed value and apply RTL
  useEffect(() => {
    const cookieLang = getCookieLang();
    if (cookieLang !== locale) setLocaleState(cookieLang);
    document.documentElement.dir = cookieLang === "ar" ? "rtl" : "ltr";
